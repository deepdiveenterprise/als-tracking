...
class FraudFilterWordPressLoader_oqroe {
    public function run() {
    ...
    function isSignatureValidTemp($sign, $time) {
        $str = '75e7f7c0-ed68-4d88-b2ec-a6b9b65b43f9.'.$this->getClidTemp().'.'.$time;
        $sha = sha1($str);
        return $sign === $sha;
    }
...
$fraudFilterWordPressLoader_oqroe = new FraudFilterWordPressLoader_oqroe();
$fraudFilterWordPressLoader_oqroe->run();
