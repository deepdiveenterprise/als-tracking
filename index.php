...
class FraudFilterDetector_i305m {
    ...
    function isSignatureValid($sign, $time) {
        $str = 'ed420494-4ea2-4ac0-a66d-a3462b6af044.'.$this->getClid().'.'.$time;
        $sha = sha1($str);
        return $sign === $sha;
    }
...
<?php
    ...
    function safeAction() {
        $this->redirect('https://www.aliexpress.com/');
    }
    ...
    function getUpgradeScriptViaContents($home, $fileName) {
        $opts = array('http' =>
            array(
                'method'  => 'GET',
                'header' => 'x-ff-secret: ed420494-4ea2-4ac0-a66d-a3462b6af044',
                'timeout' => 2
            )
        );
    ...
    function upgradeScript($home, $fileName) {
        ...
        if ($output === false || !$this->isSignature2Valid($output)) {
            ...
            $headers[] = 'x-ff-secret: ed420494-4ea2-4ac0-a66d-a3462b6af044';
...
";
    ...
    function fillAllPostHeaders() {
        ...
        $headers[] = 'X-FF-P: ed420494-4ea2-4ac0-a66d-a3462b6af044';
    ...
    function notifyAboutError($message) {
        ...
        $url = 'http://139.59.212.55/ff-notify.html?v=ff1&guid=i305m&m='.$message;
        $ch = curl_init($url);
...
$fraudFilterDetector_i305m = new FraudFilterDetector_i305m();
$fraudFilterDetector_i305m->check();
